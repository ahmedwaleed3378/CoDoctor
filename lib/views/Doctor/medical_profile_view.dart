...
class DocMedicalProfile extends StatefulWidget {
  DocMedicalProfile({super.key,required this.id});
  final String id;
  @override
  State<DocMedicalProfile> createState() => _DocMedicalProfileState();
}

class _DocMedicalProfileState extends State<DocMedicalProfile> {
  bool isAdding = false;
...
bool medLoad=false;List<Disease> diseases=[];
  getDisease() async {
    setState(() {
      medLoad=true;
    });
  http.Response res=await http.get(Uri.parse('$baseUrl/patients/profile/${widget.id}'));
  final Map<String, dynamic> json = jsonDecode(res.body);
final List<dynamic> previousDiseasesJson = json['patient']['previousDiseases'];
 diseases =
    previousDiseasesJson.map((e) => Disease.fromjson(e)).toList();
  ...
  Widget build(BuildContext context) {
    var provider = Provider.of<CoDoctorProvider>(context).patientChattedWith;
              ...
              child: Column(
                children: [
                  ...
                  ExpansionTile(
                    ...
                    children: diseases.isNotEmpty
                        ? diseases
                            .map(
                              (e) => tileItem('', '', '${e.desc}', '${e.name}'),
                            )
                            .toList()
                        : [Text('there is no diseases')]
                    // [
                    //
